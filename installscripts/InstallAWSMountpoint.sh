...
then
	if ( [ "${buildos}" = "ubuntu" ] )
	then
      if ( [ "`${HOME}/providerscripts/utilities/config/CheckBuildStyle.sh 'DATASTOREMOUNTTOOL:mountpoint'`" = "1" ] )
			then
        /usr/bin/wget https://s3.amazonaws.com/mountpoint-s3-release/latest/x86_64/mount-s3.deb
        DEBIAN_FRONTEND=noninteractive ${apt} -o DPkg::Lock::Timeout=-1  -qq -y install ./mount-s3.deb
        /bin/rm ./mount-s3.deb
		 fi
	fi
