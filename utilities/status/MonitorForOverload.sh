...
then
	...
	if ( [ "${cpu_usage}" -lt "25" ] )
	then
		${HOME}/providerscripts/datastore/config/toolkit//PutToConfigDatastore.sh ${ip} overloadedips "no"
	else
		${HOME}/providerscripts/datastore/config/toolkit/DeleteFromConfigDatastore.sh overloadedips "yes" "no"
	fi
