...
then
	...
	if ( [ "${cpu_usage}" -lt "25" ] )
	then
		${HOME}/providerscripts/datastore/operations/PutToDatastore.sh "config" "${ip}" "overloadedips" "local" "no"
	else
		${HOME}/providerscripts/datastore/operations/DeleteFromDatastore.sh "config"  "overloadedips/*" "local" 
	fi
