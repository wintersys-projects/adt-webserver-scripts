...
then
	...
	if ( [ "${cpu_usage}" -lt "25" ] )
	then
		${HOME}/providerscripts/datastore/config/wrapper//PutToConfigDatastore.sh ${ip} overloadedips "no"
	else
		${HOME}/providerscripts/datastore/config/wrapper/DeleteFromConfigDatastore.sh overloadedips "yes" "no"
	fi
