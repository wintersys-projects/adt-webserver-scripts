...
do
        if ( [ "`/bin/echo ${application_asset_dirs} | /bin/grep "WHOLE"`" != "" ] )
        then
                backup_dir=""
        fi
...
do
        ...
        if ( [ "`/bin/echo ${asset_directory} | /bin/grep -P "html(?=\s|$)"`" != "" ] )
        then
                asset_directory="/var/www/${asset_directory}"
        fi
...
do
        ...
        then
                ...
                full_path_dir_to_merge_to="`/bin/echo /var/www/html/${dir_to_merge_to} | /bin/sed 's;/html/html;/html;g'`"
