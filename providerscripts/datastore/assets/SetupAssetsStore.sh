...
do
        ...
        then
                ...
                then
                        /usr/bin/rclone mount --config /root/.config/rclone/rclone.conf-1 --allow-other --uid="${s3fs_uid}" --gid="${s3fs_gid}" --dir-cache-time 2000h --poll-interval 10s --vfs-cache-max-age 90h --vfs-cache-mode full --vfs-cache-max-size 20G  --vfs-cache-poll-interval 5m --cache-dir /home/s3mount_cache s3:${asset_bucket} ${asset_directory} &
                        count="0"
                fi
