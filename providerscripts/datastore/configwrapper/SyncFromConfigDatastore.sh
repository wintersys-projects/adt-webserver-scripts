...
then
        ...
        datastore_cmd="${datastore_tool} --config=/root/.s3cfg-1 --force --recursive --host=https://${host_base} sync --exclude-from  ${HOME}/runtime/datastore_workarea/config_datastore_sync_exclude.dat  --remove-deleted s3://${config_bucket}"
...
then
        ...
        datastore_cmd="${datastore_tool} --credentials-file /root/.s5cfg-1 --endpoint-url https://${host_base} sync --exclude '*webrootsync*' s3://${config_bucket}/"
        if ( [ "${place_to_sync}" = "" ] )
        then
                place_to_sync="*"
        fi
