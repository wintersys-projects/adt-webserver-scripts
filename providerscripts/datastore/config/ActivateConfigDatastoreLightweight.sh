...
update_to_and_from_datastore()
{
        ...
        do
                ...
                then
                        ...
                        for file in `/bin/cat ${HOME}/runtime/datastore_workarea/config/additions_to_perform.log.$$`
                        do
                                if ( [ "`/bin/echo ${file} | /bin/sed 's:/: :g' | /usr/bin/wc -w`" -gt "4" ] )
                                then
                                        place_to_put="`/bin/echo ${file} | /bin/sed 's:/[^/]*$::' | /bin/sed 's:/var/lib/adt-config/::g'`"
                                else
                                        place_to_put="root"
                                fi
                        done

                        ${HOME}/providerscripts/datastore/config/toolkit/PutToConfigDatastore.sh ${file} ${place_to_put} "no" 
