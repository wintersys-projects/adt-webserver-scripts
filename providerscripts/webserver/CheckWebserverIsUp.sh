...
then
        ...
        if ( [ "${http_online}" = "0" ] && [ "`${HOME}/providerscripts/datastore/toolkit/ListFromDatastore.sh "config" "INSTALLED_SUCCESSFULLY"`" = "INSTALLED_SUCCESSFULLY" ] )
        then
                if ( [ "${WEBSERVER_CHOICE}" = "APACHE" ] )
                then
        ...
        if ( [ "${http_online}" = "1" ] )
        then
                if ( [ "`${HOME}/providerscripts/datastore/toolkit/ListFromDatastore.sh "config" "INSTALLED_SUCCESSFULLY"`" = "INSTALLED_SUCCESSFULLY" ] && [ -f ${HOME}/runtime/WEBSERVER_READY ] )
                then
