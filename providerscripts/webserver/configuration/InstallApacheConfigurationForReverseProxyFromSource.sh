...
/bin/cp ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/apache2.conf /etc/apache2
...
then
    if ( [ -f ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/fastcgi_socket.conf ] )
    then
        /bin/sed -i -e "/XXXXFASTCGIXXXX/{r ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/fastcgi_socket.conf" -e "d}" /etc/apache2/sites-available/${WEBSITE_NAME}.conf
        /bin/sed -i "s/XXXXPHPVERSIONXXXX/${PHP_VERSION}/" /etc/apache2/sites-available/${WEBSITE_NAME}.conf
    fi
else
    if ( [ -f ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/fastcgi_port.conf ] )
    then
        /bin/sed -i -e "/XXXXFASTCGIXXXX/{r ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/fastcgi_port.conf" -e "d}" /etc/apache2/sites-available/${WEBSITE_NAME}.conf
...
if ( [ "${MOD_SECURITY}" = "1" ] )
then
        /bin/sed -i -e "/#XXXXMODSECURITYXXXX/{r ${HOME}/providerscripts/webserver/configuration/reverseproxy/apache/online/source/modsecurity.conf" -e "d}" /etc/apache2/sites-available/${WEBSITE_NAME}
fi
