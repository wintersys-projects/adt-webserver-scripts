...
then
	...
	then
		/bin/sed -i -e "/XXXXPHPXXXX/{r ${HOME}/providerscripts/webserver/configuration/${APPLICATION}/nginx/online/repo/php_socket.conf" -e "d}" /etc/nginx/sites-available/${WEBSITE_NAME}
	...
	then
		/bin/sed -i -e "/XXXXPHPXXXX/{r ${HOME}/providerscripts/webserver/configuration/${APPLICATION}/nginx/online/repo/php_port.conf" -e "d}" /etc/nginx/sites-available/${WEBSITE_NAME}
