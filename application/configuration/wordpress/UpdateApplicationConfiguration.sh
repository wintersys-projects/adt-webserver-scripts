	...
	then
		...
		then
			...
			then
				...
				if ( [ "$?" = "0" ] )
				then
					${HOME}/providerscripts/datastore/config/wrapper/PutToDatastore.sh "config" "${HOME}/runtime/wordpress_config.php" "root" "local" "yes"
				fi
			fi
	elif ( [ "`${HOME}/providerscripts/datastore/config/wrapper/ListFromDatastore.sh "config" "wordpress_config.php"`" != "" ] && [ "`/usr/bin/find ${HOME}/runtime/wordpress_config.php -cmin -1`" = "" ] )
	then
		if ( [ "`${HOME}/providerscripts/datastore/config/wrapper/AgeOfDatastoreFile.sh "config" "wordpress_config.php"`" -lt "130" ] && [ "`/usr/bin/find /var/www/html/wp-config.php -cmin -1`" = "" ] )
		then
			if ( [ -f ${HOME}/runtime/wordpress_config.php ] )
			then
			...
			${HOME}/providerscripts/datastore/config/wrapper/GetFromDatastore.sh "config" "wordpress_config.php" "${HOME}/runtime"			
