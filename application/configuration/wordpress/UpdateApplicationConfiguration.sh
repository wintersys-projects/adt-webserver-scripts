if ( [ "`${HOME}/providerscripts/datastore/configwrapper/AgeOfConfigFile.sh wordpress_config.php`" -lt "130" ] && [ "`/usr/bin/find /var/www/html/configuration.php -cmin -1`" = "" ] )
then
...
then
        ...
        then
                ...
                /bin/cp /var/www/html/configuration.php ${HOME}/runtime/wordpress_config.php
                /usr/bin/php -ln ${HOME}/runtime/wordpress_config.php
                if ( [ "$?" = "0" ] )
                then
