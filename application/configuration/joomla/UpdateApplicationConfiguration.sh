...
then
	...
	elif ( [ "`${HOME}/providerscripts/datastore/config/wrapper/ListFromDatastore.sh "config" "joomla_configuration.php"`" != "" ] && [ "`/usr/bin/find ${HOME}/runtime/joomla_configuration.php -cmin -1`" = "" ] )
 	then
		if ( [ "`${HOME}/providerscripts/datastore/config/wrapper/AgeOfDatastoreFile.sh "config" "joomla_configuration.php"`" -lt "130" ] && [ "`/usr/bin/find /var/www/html/configuration.php -cmin -1`" = "" ] )
		then
            if ( [ -f ${HOME}/runtime/joomla_configuration.php ] )
			then
			...
			${HOME}/providerscripts/datastore/config/wrapper/GetFromDatastore.sh "config" "joomla_configuration.php" "${HOME}/runtime"			
